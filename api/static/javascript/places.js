        ...
        if (placeID !== null) {
            fetch(`places/details?placeid=${encodeURIComponent(placeID)}&date=${encodeURIComponent(search_date)}&location=${encodeURIComponent(search_location)}`)
                .then(response => {
                    if (!response.ok) {   
                        popup.className = red;
                        document.getElementById('alert-text').innerHTML = "The details for this place are not available.";
                        showPopup();
                    }
...
function toggleHeartInDet() {
    ...
    .then(response => {
        if (!response.ok) {
            popup.className = red
            document.getElementById('alert-text').innerHTML = "There was an error saving the location. Please try again later"
            showPopup()
        }
        return response.json();
    })
